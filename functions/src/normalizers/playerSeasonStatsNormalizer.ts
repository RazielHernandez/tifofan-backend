...
export function normalizePlayerSeasonStats(raw: any): PlayerSeasonStats {
  ...
  return {
    ...
    // Games
    appearances: raw.games?.appearances ?? 0,
    minutes: raw.games?.minutes ?? 0,
    position: raw.games?.position ?? "",
    rating:
      raw.games?.rating != null && raw.games.rating !== "" ?
        Number(raw.games.rating) : null,
    ...
    passAccuracy:
      raw.passes?.accuracy != null && raw.passes.accuracy !== "" ?
        Number(raw.passes.accuracy) : null,
